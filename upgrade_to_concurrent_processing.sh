...
# 升级到3个并发任务处理模式脚本
# 使用方法: ./upgrade_to_concurrent_processing.sh

echo "=== 升级到3个并发任务处理模式 ==="

# 1. 备份当前文件
echo "步骤1: 备份当前clip.py文件"
cd /opt/01-vedio-hunjian/backend_py/routes
cp clip.py clip.py.backup_$(date +%Y%m%d_%H%M%S)
echo "✅ 备份完成"
...
# 6. 测试新的队列状态
echo -e "\n步骤6: 测试新的队列状态"
curl -s http://localhost:9000/api/generation/queue/status | python3 -m json.tool
