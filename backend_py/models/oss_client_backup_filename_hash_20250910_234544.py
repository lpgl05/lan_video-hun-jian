...
class OSSClient:
    ...
    def _multipart_upload(self, object_name: str, file_buffer: bytes, headers: dict = None, progress_callback = None):
        # 在分片上传前再次检查文件是否已存在（避免重复上传）        try:            if hasattr(self, '_oss_permission_checked') and self._oss_permission_checked:                self.bucket.head_object(object_name)                # 文件已存在，直接返回URL                existing_url = fhttps://{self.bucket_name}.{self.endpoint}/{object_name}                print(f🚀 分片上传前发现文件已存在，跳过上传: {existing_url})                if progress_callback:                    progress_callback(100.0, len(file_buffer), 0)                return existing_url        except oss2.exceptions.NoSuchKey:            # 文件不存在，继续分片上传            pass        except Exception as e:            print(f分片上传前检查文件存在时出错: {e}，继续上传)
        """
        分片上传实现
