...
async def process_clips(req):
    ...
    try:
        ...
        for i, clip in enumerate(composed_clips):
            ...
            try:
                # 获取GPU编码参数
                gpu_params = get_gpu_encoding_params(use_gpu=True, quality='balanced')
                
                # 提取编码器和参数
                codec = 'libx264'  # 默认CPU编码器
                ffmpeg_params = []
                
                for i, param in enumerate(gpu_params):
                    if param == '-c:v':
                        codec = gpu_params[i+1] if i+1 < len(gpu_params) else 'libx264'
                    elif param not in ['-c:v'] and not param.startswith('-'):
                        continue
                    else:
                        ffmpeg_params.extend([param, gpu_params[i+1] if i+1 < len(gpu_params) else ''])
                
                # 清理参数
                ffmpeg_params = [p for p in ffmpeg_params if p and not p.startswith('-c:v')]
                
                print(f"使用编码器: {codec}, 参数: {ffmpeg_params[:6]}...")
            ...
            try:
                ...
                video_url = f"http://39.96.187.7:9000/api/videos/oss-proxy?url={oss_url}"
        ...
        for i in range(video_count):
            ...
            try:
                success = create_optimized_video_with_ass_subtitles(
                    ...
                    portrait_mode=bool(style.get('subtitle', {}).get('position') == 'template2')
                )
...
def create_adaptive_subtitle_image(text, video_width=1080, style=None):
    ...
    if font_path and os.path.exists(font_path):
        try:
            print(f'自适应字幕使用字体: {font_path}')
            font = ImageFont.truetype(font_path, fontsize)
        except Exception as e:
            print(f'自适应字幕字体加载失败: {e}')
            font = None
...
def create_9_16_video_with_dynamic_subtitles_ffmpeg(source_video, title_image, subtitle_clips, tts_audio, bgm_audio, output_path, duration, title_position="top", subtitle_position="bottom", poster_image=None, use_gpu=True, portrait_mode: bool=False):
    ...
    if portrait_mode or subtitle_position == "template2":
        # 竖屏模式：保持原始视频宽高比，不做模糊背景，直接叠加标题与字幕
        filter_parts = [
            f"[0:v]scale={target_width}:{target_height}:force_original_aspect_ratio=decrease,pad={target_width}:{target_height}:(ow-iw)/2:(oh-ih)/2:black[base];",
    ...
    if portrait_mode or subtitle_position == "template2":
        # 竖屏模式：直接使用原始视频，保持宽高比，不进行复杂的背景前景分离
        if poster_image and poster_image != "":
            # 有海报背景：海报作为背景，原始视频直接叠加
            filter_parts = [
                f"[{poster_input_index}:v]scale={target_width}:{target_height}:force_original_aspect_ratio=decrease,pad={target_width}:{target_height}:(ow-iw)/2:(oh-ih)/2:black[bg];",
                f"[0:v]scale={target_width}:{target_height}:force_original_aspect_ratio=decrease,pad={target_width}:{target_height}:(ow-iw)/2:(oh-ih)/2:black[fg];",
                f"[bg][fg]overlay=(W-w)/2:(H-h)/2[bg_with_fg];",
                f"[bg_with_fg][1:v]overlay=0:{title_overlay_y}[with_title];"
            ]
        else:
            # 无海报背景：直接使用原始视频，保持宽高比
            filter_parts = [
                f"[0:v]scale={target_width}:{target_height}:force_original_aspect_ratio=decrease,pad={target_width}:{target_height}:(ow-iw)/2:(oh-ih)/2:black[base];",
                f"[base][1:v]overlay=0:{title_overlay_y}[with_title];"
            ]
        print(f"✅ 竖屏模式：直接使用原始视频，保持宽高比")
    else:
        # 横屏模式：使用原来的逻辑
        if poster_image and poster_image != "":
            # 有海报背景
...
def create_optimized_video_with_ass_subtitles(source_video, title_image, ass_subtitle, tts_audio, bgm_audio, output_path, duration, title_position="top", poster_image=None, use_gpu=True, subtitle_position: str = "bottom", portrait_mode: bool=False):
    ...
    if portrait_mode or subtitle_position == "template2":
        # 竖屏模式：保持原始视频宽高比，不进行背景模糊，直接叠加标题和字幕
        filter_complex = (
            f"[0:v]scale={target_width}:{target_height}:force_original_aspect_ratio=decrease,pad={target_width}:{target_height}:(ow-iw)/2:(oh-ih)/2:black[base];"
